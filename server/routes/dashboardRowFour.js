...
// Dashboard /GET
router.get("/dashboardRowFour:user", async (req, res) => {
  const arrayOfAssetArrays = [];
  let assetArrays = [];
  let benchmarkList = [];
  let returnList = [];
  ...
  benchmarkList.push(snapshots[0].dataValues.benchmark);

  // If a snapshot exists, return sums, otherwise, return 0's
  if (snapshots[0].dataValues["snapshots.id"] !== null) {
    const snapshotWithAccounts = await db.snapshots.findAll({
      where: {
        id: snapshots[0].dataValues["snapshots.id"],
      },
      include: {
    ...
    snapshotWithAccounts[0].dataValues.accounts.forEach(
      (_account, accountIndex) => {
        assetArrays = [];

        // Log each benchmark asset amount into the array
        benchmarkList.forEach((asset) => {
          // If asset isn't cash, log using normal format, otherwise log cash
          if (asset.asset !== "cash") {
            assetArrays.push(
              snapshotWithAccounts[0].dataValues.accounts[accountIndex]
                .dataValues[asset.table].dataValues[asset.asset]
            );
          } else {
            assetArrays.push(
              results[0].dataValues.accounts[accountIndex].dataValues[
                "moneyMarket"
              ]
            );
          }
...
function assetMapper(benchmarkTitle) {
  ...
  switch (benchmarkTitle) {
    ...
    case "Classic 60/40":
      assetList = [
        { table: "stock", asset: "tsm" },
        { table: "fixed_income", asset: "itb" },
      ];
      break;

    case "Three-Fund Portfolio":
      assetList = [
        { table: "stock", asset: "dlcb" },
        { table: "stock", asset: "ilcb" },
        { table: "fixed_income", asset: "itb" },
      ];
      break;
    ...
    case "No-Brainer Portfolio":
      assetList = [
        { table: "stock", asset: "dlcb" },
        { table: "stock", asset: "dscb" },
        { table: "stock", asset: "ilcb" },
        { table: "fixed_income", asset: "stb" },
      ];
      break;
    ...
    case "Ivy Portfolio":
      assetList = [
        ...
        { table: "stock", asset: "ilcb" },
        { table: "fixed_income", asset: "itb" },
        { table: "real_asset", asset: "commodoties" },
        { table: "real_asset", asset: "reits" },
      ];
      break;
    ...
    case "Permanent Portfolio":
      assetList = [
        { table: "stock", asset: "dlcb" },
        { table: "fixed_income", asset: "ltb" },
        { table: "real_asset", asset: "gold" },
        { table: "account", asset: "cash" },
      ];
      break;
