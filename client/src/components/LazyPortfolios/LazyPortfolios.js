...
class LazyPortfolios extends Component {
  static defaultProps = {
    assetInfo: {
      ...
      assetWorstDraw: ["49%", "34%", "32%", "39%", "39%", "33%", "14%", "11%"],
      assetLongestDraw: [
        "13 years",
        "12 years",
        "10 years",
  ...
  render() {
    ...
    var individualPortfolios = this.props.assetInfo.assetTitles.map(
      (data, index) => {
        return (
          <Route
    ...
    var generalPortfolios = this.props.assetInfo.assetTitles.map(
      (data, index) => {
        return (
          <Route
            key={index}
