    ...
    fetch(`/dashBoardRowFour${this.props.user}`)
      ...
      .then((response) => {
        ...
        for (
          ...
          x < this.props.benchmarkRatios.length;
          x++
        ) {
          newAssets.push("0");
        }
  ...
  computeDifferences(e) {
    ...
    for (let index = 0; index < this.props.benchmarkRatios.length; index++) {
      ...
      difference.currentAmount = (
        (difference.currentPercent / 100) *
        this.state.netWorth
      ).toFixed(2);

      difference.finalAmount = (
        (this.state.netWorth * this.props.benchmarkRatios[index]) /
        100
      ).toFixed(2);

      difference.finalPercent = (
        (difference.finalAmount / this.state.netWorth) *
        100
      ).toFixed(2);

      difference.differenceAmount = (
        (difference.currentAmount - difference.finalAmount) *
        -1
      ).toFixed(2);
    ...
    differences.forEach((object) => {
      ...
      if (
        parseFloat(this.state.rebalancePercent) <=
        Math.abs(parseFloat(object.differencePercent))
      ) {
        rebalanceCheck = true;
      }
    });
      ...
      <div class="app-content my-3 my-md-5">
        <div class="side-app">
          ...
          <form method="post" class="card">
            ...
            <div class="card-body">
              <div class="row">
                ...
                accordance with the original target risk/reward ratio. Please
                enter in a target % tolerance to analyze against. General
                reference baselines are 5%, 10%, 15%, 20%, or 25%.
                <div class="form-group" style={divStyle}>
                  ...
                  <input
                    ...
                    max="100"
                    placeholder="%"
                    onChange={(e) => this.updatePercent(e)}
                  ></input>
                  ...
                  <button
                    ...
                    style={buttonStyle}
                    onClick={(e) => this.computeDifferences(e)}
                  >
                    Analyze Portfolio
                  </button>
                </div>
          ...
          {this.state.rebalanceCheck && this.state.differences.length ? (
            <RebalanceResults
              rebalancePercent={this.state.rebalancePercent}
              differences={this.state.differences}
              needsRebalance={this.state.needsRebalance}
            />
          ) : (
            <div></div>
          )}
...
export default connect(mapStateToProps, null)(RebalanceWizard);
