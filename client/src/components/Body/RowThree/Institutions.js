...
class Institutions extends Component {
  ...
  render() {
    ...
    if (
      this.props.institutionTitle === "Tax Shelter" &&
      this.props.percentsArray
    ) {
      institutions = this.props.percentsArray.map((percent, index) => {
        return (
          <Institution
            key={this.props.shelters[index]}
            title={this.props.shelters[index]}
            percentage={percent}
          />
        );
      });
    ...
    if (
      this.props.institutionTitle === "Financial Institutions" &&
      this.props.rowThreeTotals
    ) {
    ...
    return (
      <div class="col-lg-6 col-md-12">
        <div class="card">
          ...
          <div class="card-body">
            <div class="current-progress">{institutions}</div>
          </div>
        </div>
      </div>
    );
