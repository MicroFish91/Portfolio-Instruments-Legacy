...
class RowThree extends Component {
  ...
  percentArray(rowOneObject) {
    ...
    percentArray.push(
      parseFloat((rowOneObject.taxable / rowOneObject.netWorth) * 100).toFixed(
        2
      ) + "%"
    );

    percentArray.push(
      parseFloat((rowOneObject.roth / rowOneObject.netWorth) * 100).toFixed(2) +
        "%"
    );

    percentArray.push(
      parseFloat(
        (rowOneObject.traditional / rowOneObject.netWorth) * 100
      ).toFixed(2) + "%"
    );
  ...
  render() {
    ...
    return (
      <div class="row">
        <Institutions
          rowThreeTotals={this.props.rowThreeTotals.data}
          institutionTitle={"Financial Institutions"}
        />

        <Institutions
          percentsArray={percents}
          institutionTitle={"Tax Shelter"}
        />
      </div>
    );
  }
...
export default connect(mapStateToProps, null)(RowThree);
