...
function addCore(stateArray, newObject) {
  ...
  if (accountIndex !== null) {
    newStateArray[accountIndex].amountOne = parseFloat(
      newObject.amountOne
    ).toFixed(2);
    newStateArray[accountIndex].amountTwo = parseFloat(
      newObject.amountTwo
    ).toFixed(2);
    newStateArray[accountIndex].amountThree = parseFloat(
      newObject.amountThree
    ).toFixed(2);
    newStateArray[accountIndex].amountFour = parseFloat(
      newObject.amountFour
    ).toFixed(2);
    newStateArray[accountIndex].amountFive = parseFloat(
      newObject.amountFive
    ).toFixed(2);
    newStateArray[accountIndex].amountSix = parseFloat(
      newObject.amountSix
    ).toFixed(2);

    newStateArray[accountIndex].total = objectTotal(
      newStateArray[accountIndex]
    );
  }
...
function sortAccounts(myArray) {
  var sortedLocations = myArray.map(
    (account) => account.location.toLowerCase() + account.type.toLowerCase()
  );
...
function addOther(stateArray, otherObject) {
  ...
  stateArray.forEach((object, index) => {
    if (
      object.location === otherObject.location &&
      object.type === otherObject.type
    ) {
      objectIndex = index;
